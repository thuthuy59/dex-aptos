module aptosswap::TOKEN {
    ...
    use aptosswap::utils::{ Self };
    use aptosswap::ratio::{ Self, Ratio };
    use aptosswap::sbalance:: { Self as sb, SBalance };
    use aptosswap::permission:: { Self, Permission };
    ...
    struct TokenIdo has key {
        ...
        name: vector<u8>,
        /// The price of the ido, relative to AptosCoin, in e9 format, which means when price_e9 = 10^9, 1 unit of token is selled by 1 AptosCoin (octacs)
        price_e9: u64,
        ...
        /// The collect AptosCoin token in the IDO
        collect: Balance<AptosCoin>,
    ...
    struct TokenCap has key {
        ...
        id: UID,
        /// The treasury cap for the AptosCoin Token
    ...
    public entry fun buy_token_ido_token(ido: &mut TokenIdo, inaptos: vector<Coin<AptosCoin>>, in_amount: u64, ctx: &mut TxContext) {
        do_buy_token_ido_token(ido, inaptos, in_amount, ctx);
    }
    ...
    // #[test_only]
    // public entry fun early_unlock_staked_token(bank: &mut TokenBank, farm: &mut TokenFarm, x: StakedToken,al clock: &Clock, ctx: &mut TxContext) {
    ...
    // ============================================= Utils =============================================
    fun create_stake_token(farm: &mut TokenFarm, balance: Coin<TOKEN>, profit: Coin<TOKEN>, delay_epoch: u64, unlock_epoch: u64, boost_multiplier: u64, clock: &Clock, ctx: &mut TxContext): StakedToken {
    ...
    fun destroy_stake_token(farm: &mut TokenFarm, st: StakedToken, clock: &Clock, _ctx: &mut TxContext): Coin<TOKEN> {
        let current_epoch = utils::get_epoch(clock);
    ...
    fun destroy_stake_token_force(farm: &mut TokenFarm, st: StakedToken): Coin<TOKEN> {
    ...
    fun create_linear_unlock_stake_token(balance: Coin<TOKEN>, profit: Coin<TOKEN>, delay_epoch: u64, unlock_epoch: u64, boost_multiplier: u64, clock: &Clock, ctx: &mut TxContext): LinearUnlockStakedToken {
        let id_ = object::new(ctx);
        let current_epoch = utils::get_epoch(clock);
        let start_epoch = current_epoch + delay_epoch;
        let end_epoch = start_epoch + unlock_epoch;
    ...
    fun decrease_linear_unlock_stake_token(lst: &mut LinearUnlockStakedToken, clock: &Clock, _ctx: &mut TxContext): Coin<TOKEN> {
        let current_epoch = utils::get_epoch(clock);
    ...
    fun init_impl(witness: TOKEN, ctx: &mut TxContext) {
        ...
        let (treasury_cap, coin_metadata) = coin::create_currency<TOKEN>(
            ...
            b"AptosSwap", // symbol
            b"AptosSwap Token",
            b"AptosSwap Platform Governance Token", // Description
            option::some(String::new_unsafe(ascii::string(b"https://imgs.search.brave.com/dbnsJTVVn84Msr397oEvm3-t8x9bvRLjm2YJ5Cn4ek8/rs:fit:500:0:0:0/g:ce/aHR0cHM6Ly9hc3Nl/dHMuY29pbmdlY2tv/LmNvbS9hcnRpY2xl/cy9pbWFnZXMvMTkz/NjAyNS9sYXJnZS9B/cHRvcy1BUFQtNDAw/eDIxMC5qcGc_MTcy/NjA1NjE2NQ"))),
            ctx // context
        );
    ...
    public fun do_create_token_ido(_: &Permission<TokenBank>, bank: &mut TokenBank, name: vector<u8>, price_e9: u64, is_public: bool, amount: u64, ctx: &mut TxContext) {
        ...
        let ido = TokenIdo {
            ...
            collect: balance::zero<AptosCoin>(),
            balance: sb::from_balance(sb::split(&mut bank.balance, amount)),
        };
    ...
    public fun do_buy_token_ido_token(ido: &mut TokenIdo, inaptos: vector<Coin<AptosCoin>>, in_amount: u64, ctx: &mut TxContext) {
        assert!(ido.version== VERSION, EVersionNotMatched);
        ...
        let in_balance = coin::into_balance(
            utils::merge_coins_to_amount_and_transfer_back_rest(inaptos, in_amount, ctx)
        );
        ...
        transfer::public_transfer(out_coin, signer::address_of(account));
    ...
    public fun do_unlock_linear_unlock_staked_token(x: LinearUnlockStakedToken, clock: &Clock, ctx: &mut TxContext) {
        ...
        if (is_zero_stake_token(&st)) {
            ...
            move_to::transfer(new_lst, sender);
        };
    }
    ...
    public fun do_unwrap_linear_unlock_staked_token(farm: &mut TokenFarm, x: LinearUnlockStakedToken, clock: &Clock, ctx: &mut TxContext) {
        ...
        transfer_or_destroy_zero_stake_token(st, signer::address_of(account));
    }
