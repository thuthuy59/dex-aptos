...
module aptosswap::sbalance {
    ...
    // const ESBalanceInvalidPartParameterInput: u64 = 164002;alance
    ...
    public fun from_balance<T>(balance: Coin<T>): SBalance<T> { 
    ...
    public fun into_balance<T>(x: SBalance<T>): Coin<T> {
        let SBalance { balance: balance, supply: _ } = x;
        balance
    }

    /// Increase the SBalance from the balance
    public fun increase<T>(x: &mut SBalance<T>, balance: Coin<T>) {
